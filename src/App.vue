...
export default {
  ...
  created() {
    ...
    emitter.addListener('NEW_MSG_ALERT', async (mxEvent, newRoom) => {
      // console.log('NEW_MSG_ALERT');
      // console.log(mxEvent, newRoom);
      // const sender = newRoom.members.find(e => {
      //   return e.userId === newRoom.lastMessage.senderId
      // });
      // const event = mxEvent.event;
      ...
      const otherSender = sender !== userId;
      const hasMsgType = msgContent.msgtype;
      const msgCanAlert = Object.keys(MSG_TYPE_ALERT).some(e => MSG_TYPE_ALERT[e] === msgContent.msgtype);
      const isMute = newRoom.isMute;
      const alertMe = msgContent.idlist && msgContent.idlist.indexOf(userId) > -1;
      // console.log('whether alert ? ');
      // console.log(otherSender, hasMsgType, msgCanAlert, isMute);
      if ((otherSender && hasMsgType && msgCanAlert && !isMute) || (isMute && msgContent.msgtype === MSG_TYPE.alert && alertMe)) {
